...
1. Navigate to the following GitHub repository using a web browser. 
    ...
    ![A screenshot of a cell phone Description automatically generated](../images/dp-3300-module-66-lab-05-updated.png)
...
3. You will see a screen like the one below. In order to deploy this template, you need to complete the blank fields.
    ...
    In the Resource Group field, select the existing resource group **DP300-Lab06-<inject key="Suffix"></inject>** from the drop down menu. Change the region to a region near you. For the remaining fields, use the following values:   
...
5. Upon completion, the screen will update with a link to your newly created resource. 
   ...
   Click on the **Go to resource group** link. You will be taken to the Azure Resource Group your deployment just created. You should see both a **SQL server and a SQL database** in the Resource group.  Make note of the name of your SQL server, as you will need it in the next exercise.
...
4. You'll see the screen below. Click on **Create Alert rule**.

    ![](../images/image7.png)

5. On the **Create an alert rule** page under **Condition** section click on **+ Add condition**.

    ![](../images/image6.png)

6. In the Select a signal fly out menu, select **CPU percentage**. 

    ![](../images/image8.png)

7. Supply a threshold value of **80**. Click **Actions**.

    ![Picture 15](../images/L6-EX2-T-S7.png)

8. Under the **Actions** section, click **+ Create action group**
   ...
   ![A screenshot of a social media post Description automatically generated](../images/L6-EX2-T-S8b.png)
...
10. Then click on the drop down **Notification type** and select **Email/SMS message/Push/Voice** and enter the name **DemoLab**.
    ...
    ![Picture 2059024190](../images/dp-3300-module-66-lab-20-updated.png)
...
11. In the flyout screen on the right (shown in the image above), click the check box next to email, enter an email address and click **OK**. Then click on the **Review + create** button, and then click **Create** on the Create action group screen.
    ...
    From the Create an alert rule screen, go to the Alert rule details part and add an Alert rule with the name **DemoAlert**, and then click on the **Review + create** button, and then click **Create**.
    
    **NOTE** : If any issue comes up with the resource group in the details tab, can ignore the error and proceed further to create the alert rule.
...
3. Create your automation account
   ...
   Click on **Review+Create** and then Click on **Create**. Your automation account should deploy in 1-3 minutes. 
...
4. Navigate to the **github** page for AdaptativeIndexDefragmentation. [https://github.com/microsoft/tigertoolbox/blob/master/AdaptiveIndexDefrag/usp_AdaptiveIndexDefrag.sql](https://github.com/microsoft/tigertoolbox/blob/master/AdaptiveIndexDefrag/usp_AdaptiveIndexDefrag.sql)
    ...
    Click on **Raw**.Right click on the screen select save as and save in the VM desktop locally. 
...
8. In the query editor, Select open query upload the github RAW file from the local VM that you saved in the previous steps. Delete the **USE msdb** and **GO** statements on lines 5 and 6 of the script. 
...
16.  Select the Runtime version as **5.1** from the drop down menu and then Click on **Import** in the bottom corner of the screen in order to import the module. This will import the PowerShell module into your Automation account.
    ![Picture 32](../images/dp-3300-module-66-lab-41-updated.png)
...
21. Use File Explorer within the labvm to navigate to the folder C:\LabFiles\DP-300T00-Administering-Relational-Databases-on-Azure\Allfiles\Labs\Automate Resources folder. Right click on the **DP300-Lab-Module-6.ps1** file, and click **Edit**. The PowerShell IDE will open.
