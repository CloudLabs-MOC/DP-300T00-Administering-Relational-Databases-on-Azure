...
5. Click on **+Create replica** button on the top left side. 
	...
	![Picture 20](../images/dp-3300-module-77-lab-03u.png)
...
9. Now that the Azure SQL Database is configured with replicas, you will perform a failover. Select the **Replicas** page for the secondary server and note that the primary and secondary servers are indicated.
...
3. Create the credential that will be used to access storage in the cloud with the following Transact-SQL. (If a credential already exists, drop it first.) **Fill in the appropriate values**, where *dp300storageXXXXXX* is the storage account name created in Task 1, Step 8 and *sas_token* is the value generated in Task 1, Step 10 (starting with *se=...*). **Replace XXXXXX with the Suffix value provided in the environment details page**
...
5. Back up the database WideWorldImporters to Azure with the following command in Transact-SQL, where *dp300storageXXXXXX* is the storage account name used in Task 1:**Replace XXXXXX with the Suffix value provided in the environment details page**
